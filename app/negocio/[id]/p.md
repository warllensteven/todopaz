...
const currentTime = now.getHours() _ 60 + now.getMinutes()
const [open1H, open1M] = (biz.schedule_open1 || '').split(':').map(Number)
const [close1H, close1M] = (biz.schedule_close1 || '').split(':').map(Number)
if (currentTime >= open1H _ 60 + open1M && currentTime < close1H _ 60 + close1M) return true
...
if (currentTime >= open2H _ 60 + open2M && currentTime < close2H \* 60 + close2M) return true
}
return false
}
...
{/_ Horario y Teléfono en fila _/}
...
{/_ Nota especial _/}
{biz.schedule_note && (
<div style={{ fontSize: '12px', color: 'var(--text3)', marginTop: '6px', fontStyle: 'italic' }}> \* {biz.schedule_note}
</div>
)}

{/_ Fallback por si el negocio tiene solo el campo legacy _/}
...
<div style={{ fontSize: '14px', color: 'var(--text2)' }}>{biz.schedule}</div>
    ...
    <div style={{ background: 'var(--bg)', borderRadius: 'var(--radius)', border: '1px solid var(--border)', padding: '16px 18px' }}>
      ...
      <div style={{ fontSize: '14px', color: 'var(--text)', lineHeight: 1.5 }}>+{biz.phone}</div>
    </div>
  </div>
